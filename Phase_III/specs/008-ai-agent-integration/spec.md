...
The agent maintains conversation history and can understand references to previous messages without requiring full context each time.
...
3. **Given** ongoing conversation, **When** user uses pronouns like "it" or "that", **Then** agent correctly resolves references to previous context
...
- How does agent respond to requests outside task management domain (e.g., "What's the weather?")?
...
- **FR-005**: System MUST maintain conversation history to understand contextual references
...
- **FR-012**: System MUST not access database directly - all task operations go through MCP tools
- **FR-013**: System MUST remain stateless except for conversation storage required by OpenAI Agents SDK
...
- **FR-017**: System MUST gracefully handle MCP tool failures with user-friendly error messages
- **FR-018**: System MUST support conversation turns where each turn builds on previous context

### Key Entities

- **User Message**: Natural language text input from user describing their task management intent; includes context from conversation history
...
- **Conversation Context**: Historical record of messages exchanged between user and agent; used for resolving pronouns and implicit references
...
- **SC-002**: Agent responds to task listing requests within 2 seconds
