...
- Keep architecture simple—no unnecessary abstractions
- Match existing page design system (colors, typography, spacing)
- Functional over decorative—minimalist styling for task control
...
- **Responsive**: CSS media queries + Tailwind breakpoints (≤768px mobile, >768px desktop)
...
|-----------|--------|--------------|
| **I. Multi-Tier Isolation** | ✅ PASS | All frontend code in `/frontend/`. No cross-tier imports. |
| **III. Secure by Design** | ✅ PASS | Uses Better Auth session. User ID in path, JWT auth required. |
| **IV. Zero Manual Coding** | ✅ PASS | All code generated by Claude Code following SDD workflow. |
| **VI. API Contract Enforcement** | ✅ PASS | Uses documented REST endpoint. Simple JSON request/response. |
| **X. ChatKit-Driven Conversational Interface** | ✅ PASS | **OpenAI ChatKit SDK is the core chat UI component (mandatory)**. All task operations via natural language chat. |
...
- Mobile (≤768px): Full-screen overlay when open, slide-in animation
...
├── components/
│   └── chat/
│       ├── ChatWidget.tsx        # NEW - Floating widget wrapper (contains ChatKit)
│       ├── ChatKitWrapper.tsx    # NEW - ChatKit SDK integration component
│       ├── ChatToggleButton.tsx  # NEW - Floating button to open/close
│       └── ChatTheme.tsx         # NEW - ChatKit theme configuration (optional)
├── lib/
│   ├── api/
│   │   └── chat.ts               # NEW - API client for chat endpoint
│   └── hooks/
│       └── useChatMessages.ts    # NEW - Message state + API integration
├── package.json                  # MODIFY - Add ChatKit SDK + dependencies
└── tailwind.config.ts            # MODIFY - ChatKit theme overrides (optional)
```
...
- Handles ChatKit `onSend` callback → calls API client
...
2. Create `ChatKitWrapper.tsx`
   ...
   - Implement `onSend` callback → call `useChatMessages.sendMessage()`
...
1. Create `ChatWidget.tsx` (floating widget container)
   ...
   - Responsive layout:
     ...
     - Mobile (≤768px): `fixed inset-0 w-full h-full` (full-screen)
