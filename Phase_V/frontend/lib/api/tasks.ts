 ...
 * T062: Accepts TaskFilters object; tags serialized as repeated query params.
 * GET /api/{user_id}/tasks
 */
export async function listTasks(filters?: TaskFilters): Promise<ApiResponse<TaskList>> {
  ...
  // Tags as repeated query params: ?tags=work&tags=q1
  if (filters?.tags?.length) {
    filters.tags.forEach(tag => params.append('tags', tag))
  }

  const qs = params.toString()
  return makeAuthenticatedRequest<TaskList>(`/tasks${qs ? `?${qs}` : ''}`, {
    method: 'GET',
  })
}
