...
Focus: Delegated session validation using Better Auth's native session management

Success criteria:
- Backend validates sessions by calling Better Auth's `/api/auth/session` endpoint
- Define a FastAPI dependency to forward authentication credentials to Better Auth for validation
- Extract user ID from Better Auth's session response
- Compare the user ID from the session against the '{user_id}' in the URL path
- Return 401 for invalid sessions and 403 if a user attempts to access another user's path
- Remove all custom JWT parsing and verification logic from the backend

Constraints:
- Backend delegates all session validation to Better Auth (no custom JWT verification)
- Tech stack: FastAPI + Better Auth session management
- Use environment variables for Better Auth endpoint configuration

Not building:
- Login/Signup UI logic
- MFA or password reset flows
- Custom JWT verification or token parsing logic"
...
A client application (web/mobile) authenticates a user and receives session cookies from Better Auth. The client automatically includes these session cookies when making API requests to the FastAPI backend. The backend validates the session by forwarding the request cookies to Better Auth's `/api/auth/session` endpoint and grants access to protected resources.
...
**Independent Test**: Can be fully tested by authenticating via Better Auth to obtain session cookies, making an API request with those cookies automatically included, and verifying successful access to a protected endpoint. Delivers immediate value by enabling secure API access.

**Acceptance Scenarios**:

1. **Given** a valid Better Auth session cookie, **When** the client sends a request with the session cookie to a protected endpoint, **Then** the backend validates the session by forwarding cookies to Better Auth's session endpoint and grants access with HTTP 200
2. **Given** an expired Better Auth session cookie, **When** the client sends a request with the expired session, **Then** the backend returns HTTP 401 Unauthorized with error message "Session expired"
3. **Given** an invalid Better Auth session cookie, **When** the client sends a request with the invalid session, **Then** the backend returns HTTP 401 Unauthorized with error message "Invalid session"
4. **Given** no session cookies, **When** the client sends a request to a protected endpoint, **Then** the backend returns HTTP 401 Unauthorized with error message "Missing authentication credentials"
5. **Given** a malformed or tampered session cookie, **When** the client sends the request, **Then** Better Auth's session endpoint rejects it and the backend returns HTTP 401 Unauthorized with error message "Invalid session"
...
**Independent Test**: Can be fully tested by creating two different users with valid Better Auth sessions, attempting to access each other's resources via path parameters, and verifying that cross-user access is blocked with HTTP 403. Delivers immediate security value by enforcing data isolation.
...
Developers can protect any FastAPI endpoint by adding a standardized authentication dependency. The dependency handles all credential forwarding, session validation via Better Auth, and user ID scoping logic, ensuring consistent security across all routes.
...
1. **Given** a new API endpoint decorated with the authentication dependency, **When** a request arrives without valid session credentials, **Then** the dependency automatically returns HTTP 401 before the endpoint handler executes
2. **Given** a new API endpoint decorated with the user-scoped authentication dependency, **When** a request arrives with valid session but mismatched user ID, **Then** the dependency automatically returns HTTP 403 before the endpoint handler executes
...
4. **Given** an endpoint that needs only authentication (not user ID scoping), **When** developers apply the base authentication dependency, **Then** only session validation is performed without user ID checks
...
- **FR-002**: System MUST validate sessions by calling Better Auth's `/api/auth/session` endpoint, forwarding session cookies from incoming requests
- **FR-003**: System MUST forward session cookies from incoming requests to Better Auth's session endpoint without parsing, validating, or modifying them locally
...
- **FR-009**: System MUST provide a reusable FastAPI dependency for session validation via Better Auth that can be applied to any protected route
- **FR-010**: System MUST provide a reusable FastAPI dependency for user-scoped authorization that validates both session authenticity and user ID matching
...
- **FR-013**: System MUST map Better Auth session validation responses to standardized error responses with consistent JSON structure for all authentication and authorization failures
- **FR-014**: System MUST handle missing session cookies by returning HTTP 401 with a descriptive error message
- **FR-015**: System MUST make server-to-server HTTP calls to Better Auth's session endpoint, including cookies from the original request in the outgoing HTTP call
- **FR-016**: System MUST parse the JSON response from Better Auth's session endpoint to extract user identity data (user_id) for authorization checks
- **FR-017**: System MUST NOT expose Better Auth implementation details or internal error messages directly to API clients; all errors should be translated to a consistent backend error format
...
- **SC-001**: Every protected API endpoint validates authentication within 50ms of receiving a request (session validation overhead via Better Auth API call)
- **SC-002**: System successfully rejects 100% of requests with invalid sessions (as determined by Better Auth's session endpoint) with appropriate HTTP status codes
...
- Session cookie forwarding to Better Auth's session endpoint
...
- Delegated session management to Better Auth using session cookies

**Out of Scope**:
- User login/signup UI or API endpoints (handled by Better Auth frontend integration)
- Session cookie generation and management (handled by Better Auth)
- Custom JWT verification, parsing, or signature validation (delegated to Better Auth)
- RS256/JWKS cryptographic operations (handled by Better Auth)
- Better Auth JWT plugin configuration (not required for session-based authentication)
...
**Constraints**:
- Backend MUST delegate all session validation to Better Auth via `/api/auth/session` endpoint (no custom JWT verification)
- Backend MUST NOT perform custom JWT parsing, signature verification, or JWKS operations
...
- Client applications (browsers) automatically include session cookies in requests to the backend after Better Auth authentication
...
- Network communication between client and backend uses HTTPS in production to prevent credential interception
...
- Better Auth must expose a session validation endpoint at `/api/auth/session`
...
- **Required Python libraries for backend**:
  - `httpx>=0.24.0` for making HTTP calls to Better Auth session endpoint (supports async operations and cookie forwarding)
  - `pydantic>=2.0.0` for validating session response data structure
- FastAPI framework and its dependency injection system must be available
- **Explicitly NOT required**: JWT verification libraries (PyJWT, python-jose, cryptography), JWKS libraries, or any custom cryptographic verification tools
...
- In production, both services should use HTTPS to prevent credential interception during transmission
